...
| `six_study_reliability` | Versioned synthetic manifests declare exact facts, blockers, corrections, passages, and artifacts | Two isolated clean-stack runs require 6 of 6 studies, three pre-correction export denials, deterministic agreement, and zero unsupported exported facts | `protocol_poc.reliability` | `tests/integration/reliability/test_six_study_pilot.py`, `tests/unit/reliability/test_report.py`, `tests/unit/reliability/test_cli.py` | Six bounded synthetic fixtures cannot establish reliability for untested studies, vocabularies, sections, users, or production environments. |
...
- The six-study reliability pilot covers three direct-success and three mistake-and-recovery journeys through real self-service HTTP interfaces. Two clean-stack runs must agree after run-specific data is excluded, and the invariant remains `unsupported clinical facts exported: 0`.
