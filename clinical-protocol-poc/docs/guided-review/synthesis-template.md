...
Repeated findings receive higher priority when the evidence is otherwise comparable.
...
Keep these fields distinct for every finding. Do not merge direct observations with Presenter interpretation or Reviewer requests.

| Direct observations | Presenter interpretation | Reviewer requests |
| --- | --- | --- |
| What the reviewer saw, did, or said during the presenter-controlled demonstration. | The presenter's tentative explanation, clearly marked as interpretation. | A requested change, follow-up, or capability; not evidence that it is required. |
